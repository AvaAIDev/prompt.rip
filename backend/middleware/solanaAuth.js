...
export const solanaAuth = async (req, res, next) => {
  try {
    ...
    const currentWalletAddress =
      req.headers.address || req.body?.userWalletAddress;
    ...
    if (authHeader?.startsWith("Bearer ") && authHeader.split(" ")[1]) {
      ...
      try {
        ...
        // Strict wallet address checking - token must be for the same wallet
        if (
          currentWalletAddress &&
          decoded.walletAddress !== currentWalletAddress
        ) {
          console.log(
          ...
          // Don't use this token, force re-auth with current wallet
          throw new Error("Token belongs to different wallet address");
        }
