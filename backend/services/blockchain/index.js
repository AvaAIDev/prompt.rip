...
class BlockchainService {
  ...
  async createSubmitSolutionTransaction(
    ...
    try {
      ...
      // Ensure the solution hash is exactly 32 bytes (64 hex chars)
      let solutionHashBuffer;
      if (solutionHash.length === 64) {
        // It's a hex string of the right length
        solutionHashBuffer = Buffer.from(solutionHash, "hex");
      } else {
        // It's not the right length, so hash it to make it 32 bytes
        const hashData = createHash("sha256").update(solutionHash).digest();
        solutionHashBuffer = Buffer.from(hashData);
      }

      // Make absolutely sure it's 32 bytes
      if (solutionHashBuffer.length !== 32) {
        throw new Error(
          `Invalid solution hash length: ${solutionHashBuffer.length}, expected 32 bytes`
        );
      }
      ...
      // Just use the legacy transaction approach - it's more reliable
      const transaction = new Transaction().add(instruction);
